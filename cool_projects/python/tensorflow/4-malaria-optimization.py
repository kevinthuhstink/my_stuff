...
def run_model(model, ds, epochs=1, lr=0.001, log=False, plot=False):
    ...
    # callbacks
    csv_log = keras.callbacks.CSVLogger('training_log.txt')
    training_end = keras.callbacks.EarlyStopping()
    lr_scheduler_cb = keras.callbacks.LearningRateScheduler(lr_scheduler, verbose=1)
    checkpoint = keras.callbacks.ModelCheckpoint('checkpoints/4-malaria')
    callbacks = [lr_scheduler_cb, training_end]
    if log:
...
# MAIN
def __main__():
    ds_set = handle_data(split_ratios=(0.05, 0.01, 0.01))
