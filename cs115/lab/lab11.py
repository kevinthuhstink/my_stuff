...
def insertV1(L, i):
    ...
    for pos in range( i, 0, -1 ): #note to self this is how you range
        if L[pos] < L[pos - 1]:
            swap( L, pos, pos - 1 )
        else:
            return
...
def insertV2(L, i):
    ...
    num = L[i]
    sortedPos = search( L, i, num )
    for pos in range( i - 1, sortedPos - 1, -1 ):
        swap( L, pos, pos + 1 )
    L[sortedPos] = num
...
def insertByDict( L, i, _dict ):
    '''Assume L[0:i] is sorted and 0 < i < len(L).
       Shift elements of the list as needed, and swap L[i] into
       position so that L[0:i+1] is sorted,
       using the dictionary provided by _dict to make comparisons between elements of L '''
    for pos in range( i, 0, -1 ): #note to self this is how you use range
        if _dict[ L[pos] ] < _dict[ L[pos - 1] ]:
            swap( L, pos, pos - 1 )
        else:
            return
...
def letterCounts(fname):
    ...
    if len( fdata ) == 0:
        return None
    #counts contains letters mapped to their number of occurences,
    #ex. { 'a'->2, 'r'->3 }
    for char in fdata:
        char = letter( char )
        if char not in counts:
            counts[char] = 0
        counts[char] += 1
    #insertion sort using the dict as each letter's "value"
    lettersList = list( counts.keys() ) #grab list of letters in file
    for pos in range( len( lettersList ) ): #iterate on each letter
        insertByDict( lettersList, pos, counts )
    #print values from back to front
    for i in range( len( lettersList ) - 1, -1, -1 ): #gross
        print( '(', lettersList[i], ',', counts[lettersList[i]], ')' )

#letterCounts( "smallfile.txt" )
